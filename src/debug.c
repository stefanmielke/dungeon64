...
void debug_screenshot() {
	...
	void *frame =
		(void *)(0x80000000 |
				 (*(u32 *)0xA4400004));	 // Same as calling osViGetCurrentFramebuffer() in libultra
	u32 yscale = (*(u32 *)0xA4400034);
	u32 w = (*(u32 *)0xA4400008);
	u32 h = ((((*(u32 *)0xA4400028) & 0x3FF) - (((*(u32 *)0xA4400028) >> 16) & 0x3FF)) * yscale) /
			2048;
...
void debug_printcommands() {
	...
	for (i = 0; i < debug_commands_count; i++)
		debug_printf("%d. %s\n\t%s\n", i + 1, debug_commands_elements[i].command,
					 debug_commands_elements[i].description);
	debug_printf("\n");
}
				...
				case '\0':
					if (filestep < 2) {
						if (debug_command_incoming_start[tok] != -1) {
							debug_command_incoming_size[tok] = offset -
															   debug_command_incoming_start[tok];
