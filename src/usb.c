...
#define DEBUG_ADDRESS                                                                              \
	0x04000000 - DEBUG_ADDRESS_SIZE	 // Put the debug area at the 63MB area in ROM space
...
#else
#if USE_OSRAW
	osPiRawWriteIo(D64_CIBASE_ADDRESS + D64_REGISTER_COMMAND,
				   enable ? D64_ENABLE_ROMWR : D64_DISABLE_ROMWR);
#else
	osPiWriteIo(D64_CIBASE_ADDRESS + D64_REGISTER_COMMAND,
				enable ? D64_ENABLE_ROMWR : D64_DISABLE_ROMWR);
#endif
#endif
	usb_64drive_wait();
...
#if USE_OSRAW
		osPiRawStartDma(OS_WRITE, D64_BASE_ADDRESS + DEBUG_ADDRESS + read, usb_buffer, block);
#else
		osPiStartDma(&dmaIOMessageBuf, OS_MESG_PRI_NORMAL, OS_WRITE,
					 D64_BASE_ADDRESS + DEBUG_ADDRESS + read, usb_buffer, block, &dmaMessageQ);
...
#if USE_OSRAW
	osPiRawStartDma(OS_READ, D64_BASE_ADDRESS + DEBUG_ADDRESS + usb_readblock, usb_buffer,
					BUFFER_SIZE);
#else
	osPiStartDma(&dmaIOMessageBuf, OS_MESG_PRI_NORMAL, OS_READ,
				 D64_BASE_ADDRESS + DEBUG_ADDRESS + usb_readblock, usb_buffer, BUFFER_SIZE,
				 &dmaMessageQ);
	(void)osRecvMesg(&dmaMessageQ, NULL, OS_MESG_BLOCK);
#endif
#endif
...
#else
	osPiStartDma(&dmaIOMessageBuf, OS_MESG_PRI_NORMAL, OS_READ, pi_address, buff, len,
				 &dmaMessageQ);
	(void)osRecvMesg(&dmaMessageQ, NULL, OS_MESG_BLOCK);
#endif
#endif
...
#else
	osPiStartDma(&dmaIOMessageBuf, OS_MESG_PRI_NORMAL, OS_WRITE, pi_address, buff, len,
				 &dmaMessageQ);
	(void)osRecvMesg(&dmaMessageQ, NULL, OS_MESG_BLOCK);
#endif
#endif
...
static u32 usb_everdrive_poll() {
	...
	while (len--) {
		...
		usb_everdrive_usbbusy();
		usb_everdrive_readdata(usb_buffer, ED_GET_REGADD(ED_REG_USBDAT),
							   BUFFER_SIZE);  // TODO: Replace with usb_everdrive_readusb?
...
// Set up DMA transfer between RDRAM and the PI
	...
	*(vu32 *)0xA4600010 = 3;
	...
	osWritebackDCacheAll();
#if USE_OSRAW
	osPiRawStartDma(OS_READ, ED_BASE + DEBUG_ADDRESS + usb_readblock, usb_buffer, BUFFER_SIZE);
#else
	osPiStartDma(&dmaIOMessageBuf, OS_MESG_PRI_NORMAL, OS_READ,
				 ED_BASE + DEBUG_ADDRESS + usb_readblock, usb_buffer, BUFFER_SIZE, &dmaMessageQ);
	(void)osRecvMesg(&dmaMessageQ, NULL, OS_MESG_BLOCK);
#endif
#endif
}
...
static void usb_sc64_setwritable(u8 enable) {
	...
	io_write(SC64_REG_SCR,
			 enable ? (scr | SC64_SCR_SDRAM_WRITE_EN) : (scr & (~SC64_SCR_SDRAM_WRITE_EN)));
#else
#if USE_OSRAW
	osPiRawReadIo(SC64_REG_SCR, &scr);
	osPiRawWriteIo(SC64_REG_SCR,
				   enable ? (scr | SC64_SCR_SDRAM_WRITE_EN) : (scr & (~SC64_SCR_SDRAM_WRITE_EN)));
#else
	osPiReadIo(SC64_REG_SCR, &scr);
	osPiWriteIo(SC64_REG_SCR,
				enable ? (scr | SC64_SCR_SDRAM_WRITE_EN) : (scr & (~SC64_SCR_SDRAM_WRITE_EN)));
#endif
#endif
}
...
static void usb_sc64_write(int datatype, const void *data, int size) {
	...
	while (left > 0) {
		...
		size_t data_length = MIN(MIN(left, block_size - offset),
								 usb_block_max_size - transfer_length);
		u32 dma_length;
...
// Write data to buffer in SDRAM
...
#else
		osPiStartDma(&dmaIOMessageBuf, OS_MESG_PRI_NORMAL, OS_WRITE, sdram_address, usb_buffer,
					 dma_length, &dmaMessageQ);
		...
		// Continue filling SDRAM buffer if total length is lower than maximum transfer length or if
...
#else
			osPiStartDma(&dmaIOMessageBuf, OS_MESG_PRI_NORMAL, OS_READ, SC64_MEM_USB_FIFO_BASE,
						 usb_buffer, dma_length, &dmaMessageQ);
			osRecvMesg(&dmaMessageQ, NULL, OS_MESG_BLOCK);
#endif
...
#else
	osPiStartDma(&dmaIOMessageBuf, OS_MESG_PRI_NORMAL, OS_READ, sdram_address, usb_buffer,
